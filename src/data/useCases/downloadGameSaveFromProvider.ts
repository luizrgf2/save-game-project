...
export class DownloadGameSaveFromProviderUseCase{
    ...
    async exec({gameName,outputFolder}:DownloadGameSaveFromProviderReq):Promise<Either<ErrorBase,DownloadGameSaveFromProviderRes>>{
        ...
        const downloadGameSave = await this.provider.downloadGameSave(gameSaveToDownload.right.idProvider||"",outputFolder)
